...
export function AlmacenesTable({
  ...
  const canEditPermission =
    isSuperAdmin || userPermisos.includes(PermisoBanderas.ALMACENES_EDITAR);

  const canActivatePermission =
    isSuperAdmin || userPermisos.includes(PermisoBanderas.ALMACENES_ACTIVAR);

  const canDeletePermission =
    isSuperAdmin || userPermisos.includes(PermisoBanderas.ALMACENES_ELIMINAR);
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-3 md:hidden">
        {safeAlmacenes.map((item) => {
          ...
          const isItemLoading = loadingAlmacenId === item.id;
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2 my-2.5">
                <Badge size="sm" variant="light" color="info" className="font-medium">
                  {item.tipo_almacen_nombre || `Tipo ${item.tipo_almacen}`}
                </Badge>

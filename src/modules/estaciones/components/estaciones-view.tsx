...
export function EstacionesView() {
  ...
  const hasListPermission = isSuper || userPermisos.includes(PermisoBanderas.ESTACIONES_LISTAR);

  const canCreateEstacion = isSuper || userPermisos.includes(PermisoBanderas.ESTACIONES_CREAR);
  const defaultSucursalId = currentUser?.id_sucursal_default ?? 1;

  if (currentUser && !isSuper && !hasListPermission) {
    return (
      <div>
        <PageBreadcrumb pageTitle="Estaciones" />
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 bg-white dark:bg-white/[0.03] rounded-xl border border-gray-200 dark:border-white/[0.05]">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md">
            No cuentas con el permiso requerido (<code className="font-semibold text-gray-700 dark:text-gray-300">estaciones.listar</code>) para visualizar este módulo.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <EstacionesTable
        ...
        isLoading={isLoading}
        loadingEstacionId={loadingEstacionId}
      ...
      <ConfirmDialog
        ...
        description={
          isDesactivar
            ? `¿Estás seguro de desactivar la estación '${confirmEstacion?.nombre}'?`
            : `¿Deseas activar nuevamente la estación '${confirmEstacion?.nombre}'?`
        }
